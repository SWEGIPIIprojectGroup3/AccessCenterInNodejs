...
exports.homePage = (req, res, next) => {
	...
	res.render('index', {data : result, title:'Homepage'});
	});
}
...
exports.register = async (req, res, next) => {
    ...
    try {
        ...
        dbConn.query(query2, [body.email], async(error, row)=>{
			...
			var query3 = "INSERT INTO `users`(`firstName`,`lastName`,`username`, `mobile`,`email`,`password`,`gender`) VALUES(?,?,?,?,?,?,?)";
			dbConn.query(query3, [body.firstName, body.lastName, body.username, body.mobile, body.email, hashPass, body.gender], 
						 (error, rows)=>{
							if(error)
							{
								console.log (error);
